...
class sakt(nn.Module):  
    def __init__(self , ex_total , seq_len, dim , heads ):
        ...
        self.embd_in = nn.Embedding( ex_total, embedding_dim  = dim )         # Interaction embedding
        self.embd_ex = nn.Embedding( ex_total , embedding_dim = dim )       # Excercise embedding  
        ...
        self.linear_out = nn.Linear(in_features= dim , out_features= 1 , bias=True)                            # output with correctnness prediction 

    def forward( self , input_in , input_ex):

        ## positional embedding
        pos_in = self.embd_pos( torch.arange(12).unsqueeze(0) )         #making a tensor of 12 numbers, .unsqueeze(0) for converting to 2d, so as to get a 3d output #print('pos embd' , pos_in.shape)
        ...
        atn_out , _ = self.attn(query_ex , key_in, value_in , 
                                attn_mask= torch.from_numpy( np.triu(np.ones((n,n)), k=1).astype('bool')) )      # lower triangular mask, bool, torch    (n,b,d)
        ...
        atn_out = nn.LayerNorm(d)( atn_out )                          # Layer norm                        #print('atn',atn_out.shape) #n,b,d = atn_out.shape

        #take batch on first axis 
        atn_out = atn_out.permute(1,0,2)                              #  (n,b,d) --> (b,n,d)
        
        ## FFN 2 layers
        ffn_out = self.ffn[1]( nn.ReLU()( self.ffn[0]( atn_out ) ))   # (n,b,d) -->    .view([n*b ,d]) is not needed according to the kaggle implementation
        ffn_out = nn.LayerNorm(d)( ffn_out + atn_out )                # Layer norm and Residual connection

        ## sigmoid
        ffn_out = nn.Sigmoid()(self.linear_out( ffn_out )  )
